                    ...
                    <form class="user" action="">
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-sm-6" align="right">
                            <a href="<?php echo e(route('rm.tambah.id',$iden->id)); ?>" class="d-none d-sm-inline btn btn-primary shadow-sm">
    ...
    <div class="card shadow mb-4">
                ...
                <table class="table table-bordered table-striped" id="pasien" width="100%" cellspacing="0">
                  ...
                  <?php $__currentLoopData = $rms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rm): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr>
                      <td><?php echo e(str_pad($rm->id, 4, '0', STR_PAD_LEFT)); ?></td>
                      <td><?php echo e(format_date($rm->created_time)); ?></td>
                      <td><?php echo e($rm->ku); ?></td>
                      <td>
                      <?php if($rm->lab != NULL): ?>
                        <?php for($i=0;$i<sizeof($lab=encode($rm->lab));$i++): ?>
                            <?php if($has=encode($rm->hasil)): ?>
                                <li><?php echo e(get_value('lab',$lab[$i],'nama')); ?> : <?php echo e($has[$i]); ?> <?php echo e(get_value('lab',$lab[$i],'satuan')); ?></li>
                            <?php endif; ?>
                        <?php endfor; ?>
                      <?php endif; ?>
                      </td>
                      ...
                      <td>
                      <?php if($rm->resep != NULL): ?>
                        <?php for($i=0;$i<sizeof($resep=encode($rm->resep));$i++): ?>
                            <?php if($aturan=encode($rm->aturan)): ?>
                                <li><?php echo e(get_value('obat',$resep[$i],'nama_obat')); ?> <?php echo e(get_value('obat',$resep[$i],'sediaan')); ?> <?php echo e(get_value('obat',$resep[$i],'dosis')); ?> <?php echo e(get_value('obat',$resep[$i],'satuan')); ?> : <?php echo e($aturan[$i]); ?></li>
                            <?php endif; ?>
                        <?php endfor; ?>
                      <?php endif; ?>
                      </td>
                      <td width="120px">
                        <a href="<?php echo e(route('rm.edit', $rm->id)); ?>" class="btn btn-warning btn-sm btn-icon-split">
                        <span class="icon">
                        <i style="padding-top:4px"class="fas fa-pen"></i>
                        </span>
                        <span class="text">Edit</span>
                        </a>
                        <a href="<?php echo e(route('rm.lihat', $rm->id)); ?>" class="btn btn-success btn-sm btn-icon-split">
                        <span class="icon">
                        <i style="padding-top:4px"class="fas fa-eye"></i>
                        </span>
                        <span class="text">Lihat</span>
                        </a>
                        <a href="<?php echo e(route('tagihan', $rm->id)); ?>" class="btn btn-secondary btn-sm btn-icon-split">
                        <span class="icon">
                        <i style="padding-top:4px"class="fas fa-cart-plus"></i>
                        </span>
                        <span class="text">Tagihan</span>
                        </a>
                        <a href="javascript:;" data-toggle="modal" onclick="deleteData(<?php echo e($rm->id); ?>)" data-target="#DeleteModal" class="btn btn-sm btn-icon-split btn-danger">
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\SKlinik\resources\views/list-rm.blade.php ENDPATH**/ ?>